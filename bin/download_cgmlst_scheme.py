...
cgmlst_schemes = {
    ...
    "clostridioides": {
        "source": "seqsphere",
        "url": "https://www.cgmlst.org/ncs/schema/12556067/locus/",
    },
    "shigella": {
        "source": "enterobase",
...
class cgMLSTSchemes:
    ...
    def __init__(self, genus_list, output_dir="output", threads=2, download_loci=True):
        ...
        genus_to_download = [
            genus for genus in self.genus_list if genus in self.schemes
        ]
        genus_to_download = list(set(genus_to_download))
        genus_not_found = [
            genus for genus in self.genus_list if genus not in self.schemes
        ]
        ...
        if all_nonsupported:
            supported_genera = [
                genus
                for genus in cgmlst_schemes.keys()
        ...
        downloaded_schemes = {
            genus: info
            for (genus, info) in self.download_cgmlst_scheme(
                genus_list=genus_to_download
            )
        }
        self.schemes = {
    ...
    def download_cgmlst_scheme(self, genus_list):
        for genus in genus_list:
            ...
            if source == "pubmlst":
                genus_scheme_info = self.download_pubmlst_scheme(
                    scheme_url, output_dir_per_genus, genus
                )
                ...
                genus_scheme_info = self.download_enterobase_scheme(
            ...
            if self.download_loci:
                ...
                genus_scheme_info["genus"] = genus
                with open(
                    output_dir_per_genus.joinpath("downloaded_scheme.yaml"), "w"
                ) as file_handle:
                    file_handle.write(
                        yaml.dump(genus_scheme_info, default_flow_style=False)
                    )
...
def main():
    ...
    choices_scheme = [
        genus for genus in cgmlst_schemes.keys() if not genus.startswith("test_")
    ]
    ...
    argument_parser.add_argument(
        "--no_download", dest="download_loci", action="store_false"
    )
    args = argument_parser.parse_args()
    cgMLSTSchemes(
        threads=args.threads,
        genus_list=args.genus,
