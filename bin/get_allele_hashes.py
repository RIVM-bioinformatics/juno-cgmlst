...
class Locus:
    ...
    def __init__(self, fasta_file=None, name="", alleles={}):
        self.name = str(name)
        ...
        if fasta_file is not None:
            ...
            with open(fasta_file, "r") as file_:
                ...
                if not any(fasta_record):
                    raise ValueError(
                        f"The provided fasta file {self.fasta_file} does not have the expected fasta format."
                    )
    ...
    def get_all_alleles_from_fasta(self):
        with open(self.fasta_file, "r") as file_:
            ...
            list_alleles = [
                self.__get_id_obj_pairs_from_fasta_record(record)
    ...
    def make_table_with_hashes(self, subset=False):
        ...
        if subset:
            hash_tuple_list = [
                (x.id_number, x.hash) for x in self.allele_subset.values()
            ]
        else:
...
class CgmlstResult:
    ...
    def __init__(
        ...
        scheme_name,
        db_dir="/mnt/db/juno/cgmlst/prepared_schemes",
        threads=2,
    ):
    ...
    def read_chewbbaca_result_file(self):
        ...
        self.cgmlst_result = pd.read_csv(
            self.chewbbaca_result_file, sep="\t", index_col="FILE", dtype=str
        )
    ...
    def get_db_dir(self, db_dir, scheme_name):
        ...
        if self.__validate_db_dir(db_dir):
            ...
            raise ValueError(
                f"No fasta files were found in the provided db_directory ({db_dir}) or any of its expected subfolders ({db_dir_genus} or {db_dir_scheme_folder})"
            )
    ...
    def __get_hash_from_hash_id_tbl(self, hash_id_tbl, id_number):
        allele_hash = hash_id_tbl.loc[
            hash_id_tbl["id_number"] == id_number, "hash"
        ].tolist()
    ...
    def __gen_hash_per_id_num(self, hash_id_tbl, id_numbers, fasta_file):
        for id_num in id_numbers:
            try:
                ...
                try:
                    allele_hash = self.__get_hash_from_hash_id_tbl(
                        hash_id_tbl=hash_id_tbl, id_number=id_num_int
                    )
                except IndexError:
    ...
    def get_hashes_for_locus(self, locus, id_numbers=[]):
        ...
        try:
            hash_id_tbl = pd.read_csv(
                csv_translation, dtype={"id_number": int, "hash": str}
            )
        except FileNotFoundError:
            locus_obj = Locus(fasta_file=fasta_file)
            locus_obj.get_all_alleles_from_fasta()
            hash_id_tbl = locus_obj.make_table_with_hashes()
        subset_allele_hashes = [
            x for x in self.__gen_hash_per_id_num(hash_id_tbl, id_numbers, fasta_file)
        ]
    ...
    def __gen_hash_per_locus(self, loci_list):
        for locus in loci_list:
            ...
            allele_hashes = self.get_hashes_for_locus(
                locus=locus, id_numbers=id_numbers
            )
...
def main(input, scheme_name, db_dir, output, threads):
    print(f"Initializing...")
    chewbbaca_res = CgmlstResult(
        chewbbaca_result_file=input,
...
if __name__ == "__main__":
    ...
    argument_parser.add_argument(
        "input", type=pathlib.Path, help="Csv file containing the results of ChewBBACA."
    )
    argument_parser.add_argument(
        "-s",
        "--scheme-name",
        ...
        type=str,
        choices=[
            scheme for scheme in cgmlst_schemes.keys() if not scheme.startswith("test_")
        ],
        required=True,
    ...
    argument_parser.add_argument(
        ...
        default="/mnt/db/juno/cgmlst/prepared_schemes",
    )
    argument_parser.add_argument(
        "-o",
        "--output",
    ...
    argument_parser.add_argument(
        "-t", "--threads", type=int, default=4, help="Number of threads to be used."
    )
