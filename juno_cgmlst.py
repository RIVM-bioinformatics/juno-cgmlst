...
class JunoCgmlstRun(base_juno_pipeline.PipelineStartup,
    ...
    def __init__(self, 
        ...
        base_juno_pipeline.RunSnakemake.__init__(self,
            ...
            restarttimes=1,
            latency_wait=60,
            name_snakemake_report=str(self.path_to_audit.joinpath('juno_cgmlst_report.html')),
            **kwargs)
    ...
    def run_juno_cgmlst_pipeline(self):
        ...
        if not self.dryrun or self.unlock:
            subprocess.run(
                f'find {self.output_dir} -type f -empty -exec rm {{}} \;',
                shell=True
            )
            subprocess.run(
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help = "Relative or absolute path to the directory that contains the databases for all the tools used in this pipeline or where they should be downloaded. Default is: /mnt/db/juno/cgmlst_db",
    )
    parser.add_argument(
        "-c",
        "--cores",
